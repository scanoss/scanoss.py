...
class TestScanFilter(unittest.TestCase):
    def setUp(self):
        self.settings = {
            "skip": {"patterns": [], "sizes": {"min": 0, "max": float("inf")}}
        }
        self.scan_filter = ScanFilter(settings=self.settings, debug=True)
    ...
    @patch("os.walk")
    def test_skip_directories(self, mock_walk):
        mock_walk.return_value = [
    ...
    @patch("os.walk")
    def test_custom_skip_patterns(self, mock_walk):
        self.scan_filter.skip_patterns.append("*.md")

        mock_walk.return_value = [
            ("/scan_root", [], ["file1.txt", "file2.md", "file3.py"]),
        ]

        expected_files = ["./file1.txt", "./file3.py"]
